...
class informationNetwork():
	...
	def __init__(self, rand_int=0, num_of_samples=None, args=None):
		...
		self.bn=args.batch_norm
		self.l2=args.l2
		self.l2_pen=args.penalty
		...
		self.ws, self.grads, self.information, self.models, self.names, self.networks, self.weights,self.acc, self.train_acc = [
			[[[[None] for k in range(len(self.train_samples))] for j in range(len(self.layers_sizes))]
			 for i in range(self.num_of_repeats)] for _ in range(9)]
		...
		params = {'sampleLen': len(self.train_samples), 'activation':self.activation_function,
		          'nDistSmpls': args.nDistSmpls, 'BatchNorm': args.batch_norm,
		          ...
		          'lr': args.learning_rate,'random_labels': args.random_labels,'regularization':args.l2}
		if args.l2:
	...
	def save_data(self, i='',parent_dir='jobs/', file_to_save='data.pickle'):
		...
		data = {'information': self.information,
                ...
                'l1_norms': self.l1_norms, 'ws': self.ws, 'activation':self.activation_function, 
                'layer_configurations': self.layers_sizes,
            'input_size':self.data_sets.data.shape[1],'num_of_classes':self.data_sets.labels.shape[1],
            'learning_rate':self.learning_rate,'cov_net':self.cov_net,
            'test_accuracy':self.acc,'train_accuracy':self.train_acc,
            'data':self.data_sets.data, 'labels':self.data_sets.labels}

		if not os.path.exists(directory):
			os.makedirs(directory)
		self.dir_saved = directory
		with open(self.dir_saved+str(i)+file_to_save, 'wb') as f:
	...
	def run_network(self):
		...
		if self.run_in_parallel:
			...
			results = [nn.train_and_calc_inf_network(i, j, k,
			                                         ...
			                                         self.save_ws, self.rand_int,self.bn, self.cov_net, self.l2,self.l2_pen)
		...
		for i in range(len(self.train_samples)):
			for j in range(len(self.layers_sizes)):
				for k in range(self.num_of_repeats):
					...
					self.acc[k][j][i]=current_network['test_acc']
					self.train_acc[k][j][i]=current_network['train_acc']
	...
	def calc_information(self):
		...
		if self.traind_network and self.save_ws:
			self.information = np.array(
				[inn.get_information(self.activation_function,self.ws[k][j][i], self.data_sets.data, self.data_sets.labels,
	...
	def calc_information_last(self):
		...
		if self.traind_network and self.save_ws:
			return np.array([inn.get_information([self.activation_function, self.ws[k][j][i][-1]], self.data_sets.data, self.data_sets.labels,
