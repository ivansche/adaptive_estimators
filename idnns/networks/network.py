...
def build_model(activation_function, layerSize, input_size, num_of_classes, learning_rate_local, save_file, batch_norm,covn_net,l2, penalty):
	...
	model = mo.Model(input_size, layerSize, num_of_classes, learning_rate_local, save_file, int(activation_function),
	                 batch_norm=batch_norm,cov_net=covn_net,l2=l2, penalty=penalty)
	return model
...
def train_and_calc_inf_network(i, j, k, layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
                               ...
                               interval_information_display, save_ws, rand_int,batch_norm, cov_net, l2, penalty):
	...
	network = train_network(layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
	                        ...
	                        batch_norm,cov_net,l2, penalty)
	network['information'] = []
	if calc_information:
		print ('Calculating the infomration')
		infomration = np.array([inn.get_information(model_type,network['ws'], data_sets_org.data, data_sets_org.labels,
		...
		infomration = np.array([inn.get_information(model_type,[network['ws'][-1]], data_sets_org.data, data_sets_org.labels,
...
def exctract_activity(sess, batch_points_all, model, data_sets_org, batch_norm):
	...
	for i in range(0, len(batch_points_all) - 1):
		...
		if batch_norm:
		               feed_dict_temp[model.phase]= False
	...
	print("~~~~~~~~~~~~~~~~~~~~")
	print(w_temp[-1])
	print(softmax(w_temp[-1]))
	print("~~~~~~~~~~~~~~~~~~~~")
	raise SystemError
	'''
	for k in range(len(w_temp[-1])):
		w_temp[-1][k]=softmax(w_temp[-1][k])
	return w_temp
...
def print_accuracy(batch_points_test, data_sets, model, sess, j, acc_train_array, batch_norm):
	...
	return np.mean(np.array(acc_array)),np.mean(np.array(acc_train_array))
...
def train_network(layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
                  ...
                  name, batch_norm,covn_net,l2, penalty):
	"""Train the nework"""
	...
	# Build the network
	model = build_model(model_type, layerSize, input_size, num_of_classes, learning_rate_local, name,batch_norm, covn_net,l2, penalty)
	...
	with tf.Session() as sess:
		...
		for j in range(0, num_of_ephocs):
			epochs_grads = []
			if j in indexes:
				ws[k] = exctract_activity(sess, batch_points_all, model, data_sets_org, batch_norm)
			# Print accuracy
			if np.mod(j, interval_accuracy_display) == 1 or interval_accuracy_display == 1:

				ap,atp=print_accuracy(batch_points_test, data_sets, model, sess, j, acc_train_array,batch_norm)
				acc_pers.append(ap)
				acc_train_pers.append(atp)
	...
	network['model'] = model
	network['train_acc'] = acc_train_pers
	network['test_acc']=acc_pers
	return network
